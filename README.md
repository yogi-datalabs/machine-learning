...
16. **PyCaret** — `PyCaret/` — manual model-comparison loop vs. PyCaret's AutoML, verified to agree
    on the same held-out test set (+ Teaching Notes). **Runs in its own virtual environment**, see below.
17. **Model Explainability** — `Model Explainability/` — explaining the loan-default Gradient
